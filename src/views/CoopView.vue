...
// 收藏的 store
const { getList, toggle } = useBookmarkStore()
const { state } = useRentalStore()

// 收藏的區域場地
const bookmarkedVenues = computed(() => {
  const areaIds = getList('area')
  return areaIds
...
// 收藏的廣告位
const bookmarkedAds = computed(() => {
  const adIds = getList('ad')
  return adIds
    .map((id) => ({
      id,
      item: adData.find((ad) => ad.id === id)
    }))
    .filter((x) => x.item)
})
...
const removeBookmark = (id, type, name) => {
  Swal.fire({
    title: '取消收藏',
    text: `確定要將「${name}」從收藏中移除嗎？`,
    icon: 'question',
    showCancelButton: true,
    confirmButtonColor: '#C62828',
    cancelButtonColor: '#0D2953',
    confirmButtonText: '是的，移除',
    cancelButtonText: '取消'
  }).then((result) => {
    if (result.isConfirmed) {
      toggle(id, type) // ✅ 呼叫 toggle 取消收藏
        ...
        <div class="p-5 md:p-15">
          ...
          <div class="mt-10">
            ...
            <div class="mt-5">
              ...
              <div class="mt-2 flex flex-wrap gap-3">
                <!-- 區域場地 -->
                <HashTag
                  v-for="{ id, item } in bookmarkedVenues"
                  :key="`venue-${id}`"
                  @click="removeBookmark(id, 'area', item.name)"
                  class="cursor-pointer transition-all hover:scale-95 hover:opacity-70"
                  :class="'group-hover:bg-white group-hover:text-[#214F83]'"
                >
                  {{ item.tag }}
                </HashTag>
                ...
                <!-- 廣告位 -->
                <HashTag
                  v-for="{ id, item } in bookmarkedAds"
                  :key="`ad-${id}`"
                  @click="removeBookmark(id, 'ad', item.name)"
                  class="cursor-pointer transition-all hover:scale-95 hover:opacity-70"
                  :class="'group-hover:bg-white group-hover:text-[#214F83]'"
                >
                  {{ item.tag }}
                ...
                <div
                  v-if="
                    bookmarkedVenues.length === 0 && bookmarkedAds.length === 0
                  "
                  class="text-gray-400"
                >
                  尚無收藏項目
          ...
          >
            <div class="flex flex-col gap-5 md:flex-row">
              <div
                class="cursor-pointer p-3 text-center shadow-[3px_3px_5px_rgba(0,0,0,0.4)] transition-all hover:bg-gray-50 active:scale-95 md:w-[200px] md:text-[16px] lg:w-[240px] lg:text-[20px]"
              >
                進徹場說明手冊
                <BaseIcon
      ...
      <div class="hidden p-5 sm:block md:p-8 lg:p-10 xl:p-15">
        ...
        <div class="mt-5 h-[500px] w-full">
          <swiper
            ...
            class="h-[400px] w-full text-white [--swiper-navigation-color:#fff] [--swiper-navigation-size:30px] md:h-[500px] lg:h-[600px]"
          >
            <swiper-slide
              v-for="photo in roomPhotos"
              :key="photo.id"
              class="flex items-center justify-center transition-all duration-300 [--swiper-navigation-color:white] [--swiper-navigation-size:30px]"
            >
              <img
                :src="photo.src"
                :alt="photo.alt"
