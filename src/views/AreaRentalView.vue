...
<template>
  ...
  <GeneralFrame titleLeft="租借" titleRight="服務" subTitle1="區域租借">
    ...
    <div class="relative w-full h-40 md:h-40">
      <div class="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-[65%] w-3/4 h-auto shadow-[2px_4px_10px_rgba(0,0,0,0.25)] bg-white 
      flex flex-col items-center justify-center 
      gap-3 md:gap-12 
      py-4 sm:py-10 md:py-10 
      lg:py-15  px-5 md:px-10">
        <p class="font-inter font-bold text-[16x] md:text-[24px]">從現在開始，選擇容納人數與用途hashtag，一步一步，把想法變成一個真的會發生的畫面。</p>
        <SearchBar v-model:guests="state.draftGuests" v-model:hashtag="state.draftHashtag" @search="selectFilters" />
      </div>
    </div>

    <!-- Cards -->
    <div class="w-full  bg-white rounded-[16px] pt-75 md:pt-60 pb-10 px-2 md:px-14 ">
      <!-- 有搜尋結果 -->
